...
export default function VectorSubtractionCalculatorPage() {
  const [vector1, setVector1] = useState("4, 5, 6")
  const [vector2, setVector2] = useState("1, 2, 3")
  ...
  return (
    <div className="flex justify-center items-center py-8">
      <Card className="w-full max-w-md">
        <CardHeader>
          <CardTitle className="text-2xl">Vector Subtraction Calculator</CardTitle>
          <CardDescription>Enter two vectors of the same dimension.</CardDescription>
        </CardHeader>
        <CardContent className="grid gap-4">
          <InputWithLabel label="Vector 1" value={vector1} onChange={setVector1} placeholder="e.g., 4, 5, 6"/>
          <InputWithLabel label="Vector 2" value={vector2} onChange={setVector2} placeholder="e.g., 1, 2, 3"/>
          <Button onClick={calculate}>Subtract Vectors</Button>
