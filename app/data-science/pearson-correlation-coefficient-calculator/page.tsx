...
export default function CorrelationCalculatorPage() {
  ...
  const [numbers2, setNumbers2] = useState("1, 2, 3, 4, 5")
  ...
  const calculate = () => {
    ...
    const stdDev1 = Math.sqrt(arr1.reduce((a, b) => a + Math.pow(b - mean1, 2), 0) / (arr1.length -1));
    const stdDev2 = Math.sqrt(arr2.reduce((a, b) => a + Math.pow(b - mean2, 2), 0) / (arr2.length -1));
    ...
    setResult(correlation.toFixed(4));
    setError("");
  }
  ...
  return (
    <div className="flex justify-center items-center py-8">
      <Card className="w-full max-w-md">
        <CardHeader>
          <CardTitle className="text-2xl">Pearson Correlation Coefficient Calculator</CardTitle>
        ...
        <CardContent className="grid gap-4">
          ...
          {result && (
            <div className="p-4 bg-muted rounded-md text-center">
                <p>Pearson Correlation (r): <span className="font-bold">{result}</span></p>
            </div>
          )}
        </CardContent>
      </Card>
